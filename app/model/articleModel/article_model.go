...
// arModel is a active record design model for table article operations.
type arModel struct {
	gmvc.M
}

var (
	// Table is the table name of article.
	Table = "article"
	// Model is the model object of article.
	Model = &arModel{g.DB("default").Table(Table).Safe()}
	// Columns defines and stores column names for table article.
	Columns = struct {
		Id          string //
		MeId        string //
		Title       string //
...
// 根据ID查找第一个
func (m *arModel) First(id int) (*Entity, error) {
	one, err := m.M.FindOne("id = ?", id)
	if err != nil {
		return nil, err
	}
	var entity *Entity
	...
	if entity == nil {
		return nil, errors.New("data no exist")
	}
	if err != nil {
		glog.Error(err)
